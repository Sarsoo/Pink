...
namespace Pink.Mechanics
{
	public class CharacterKinematics : MonoBehaviour
	{
		...
		public void Move(float move, bool crouch, bool jump)
		{
			...
			else if (!m_Grounded && jump && !m_hasDoubleJumped)
			{
				m_hasDoubleJumped = true;

                //Vector3 targetVelocity = new Vector2(move * 10f, m_DoubleJumpForce);
                m_Rigidbody2D.velocity = new Vector2(move * 10f, m_DoubleJumpForce);
                // And then smoothing it out and applying it to the character
                //m_Rigidbody2D.velocity = Vector3.SmoothDamp(m_Rigidbody2D.velocity, targetVelocity, ref m_Velocity, m_MovementSmoothing);
